...
function update(MaincategoriesRequest $request,$id){
    ...
    if(!$data){
        ...
        if(isset($Maincategories['photo'])){
            ...
            $path= uploud_file("maincatigory",  $Maincategories['photo']->hashName(), $Maincategories['photo']);
            $Maincategories['photo']=$path;
             $data->update(
                $Maincategories
              );
        }
...
function delete($id){
   ...
   $d=  Main_categrories::with('categrories')->find($id);

   if($d){
    try{
    $img=str_replace("storage","storage/app/public",$d->photo);
